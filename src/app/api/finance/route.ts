...
function extractProduit(raw: unknown): {
  nom_produit: string
  categorie: string
...
export async function GET(request: Request) {
  try {
    ...
    // =============================================
    // SOLDE GLOBAL (depuis le debut, toutes dates)
    // =============================================

    const { data: tousEncaissements } = await supabase
      .from('encaissements')
      .select('montant_encaisse')
      .eq('societe_id', societe_id)

    let total_encaissements = 0
    if (tousEncaissements) {
      for (const e of tousEncaissements) {
        total_encaissements += Number(e.montant_encaisse || 0)
      }
    }

    const { data: tousRecouvrements } = await supabase
      .from('recouvrements')
      .select('montant_recouvre')
      .eq('societe_id', societe_id)
      .eq('is_archived', false)

    let total_recouvrements = 0
    if (tousRecouvrements) {
      for (const r of tousRecouvrements) {
        total_recouvrements += Number(r.montant_recouvre || 0)
      }
    }

    const { data: toutesDepenses } = await supabase
      .from('depenses')
      .select('montant')
      .eq('societe_id', societe_id)
      .eq('is_archived', false)
    ...
    if (toutesDepenses) {
      for (const d of toutesDepenses) {
        total_depenses_global += Number(d.montant || 0)
      }
    }

    const { data: tousMovements } = await supabase
      .from('mouvements_caisse')
      .select('montant, type_mouvement')
      .eq('societe_id', societe_id)
      .eq('is_archived', false)
    ...
    if (tousMovements) {
      for (const m of tousMovements) {
        if (m.type_mouvement === 'INJECTION') {
          total_injections += Number(m.montant || 0)
        } else {
          total_retraits += Number(m.montant || 0)
        }
      }
    }
    ...
    const solde_global =
      ...
      total_injections -
      total_retraits -
      total_depenses_global

    // =============================================
    // DEPENSES SUR PERIODE FILTREE
    // =============================================
    ...
    if (depensesPeriode) {
      for (const d of depensesPeriode) {
        ...
        if (d.categorie === 'MATIERES_PREMIERES') {
          depenses_matieres += montant
        } else {
          depenses_hors_matieres += montant
        }
      }
    }
    ...
    const { data: encaissementsPeriode } = await supabase
      .from('encaissements')
      .select('montant_encaisse')
      .eq('societe_id', societe_id)
    ...
    let ca_periode = 0
    if (encaissementsPeriode) {
      for (const e of encaissementsPeriode) {
        ca_periode += Number(e.montant_encaisse || 0)
      }
    }

    // =============================================
    // BENEFICE PAR PRODUIT SUR PERIODE
    // =============================================
    ...
    // =============================================
    // GRAPHIQUE 6 MOIS (CA vs Depenses)
    // =============================================
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      graphique_mois.push({
        mois: labelMois,
        ca: caMois,
        depenses: depMoisTotal,
        benefice: caMois - depMoisTotal,
      })
    }
    ...
    const { data: mouvementsRecents } = await supabase
      .from('mouvements_caisse')
      .select('*')
      .eq('societe_id', societe_id)
      .eq('is_archived', false)
    ...
    return NextResponse.json({
      ...
      total_depenses_global,
      ca_periode,
      total_depenses_periode,
      depenses_matieres,
      depenses_hors_matieres,
      marge_brute,
      benefice_net,
