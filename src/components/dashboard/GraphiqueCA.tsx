...
function CustomTooltip(props: TooltipProps) {
  ...
  const labelText = props.label ? formatDate(props.label) : ''

  return (
    <div className="bg-white border border-gray-200 rounded-lg px-3 py-2 shadow-lg">
      <p className="text-xs text-gray-500 mb-1">{labelText}</p>
      <p className="text-sm font-semibold text-blue-600">
        {Number(props.payload[0].value).toLocaleString('fr-MG')} Ar
      </p>
    </div>
  )
}

export default function GraphiqueCA(props: GraphiqueCAProps) {
  const aDesData = props.data.some(function (d) {
    return d.ca > 0
  })
