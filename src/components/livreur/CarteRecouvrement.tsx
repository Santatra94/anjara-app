...
import { GripVertical, MapPin, Phone, ArrowRight, Wallet } from 'lucide-react';
import Link from 'next/link';

export function CarteRecouvrement({ task }: { task: TourneeDuJour }) {
  const {
    attributes,
    listeners,
    setNodeRef,
            ...
            <div className="flex-1 p-4 flex flex-col justify-between">
              <div className="space-y-1">
                <div className="flex justify-between items-start">
                   <Badge className="bg-orange-500 text-white border-none rounded-full text-[9px] px-2 font-black uppercase">
                     <Wallet className="h-2 w-2 mr-1 inline" /> Recouvrement
                   </Badge>
                </div>
              ...
              <div className="flex items-center justify-between pt-4">
                <div>
                    <span className="text-[9px] font-black text-gray-400 uppercase tracking-tighter block mb-0.5">Dette à recouvrer</span>
                    <div className="text-orange-600 font-black text-lg">
                    {task.dette_restante?.toLocaleString()} <span className="text-[10px]">Ar</span>
                    </div>
                </div>
                ...
                <div className="flex gap-2">
                    ...
                    <Button
                        asChild
                        className="h-10 rounded-full bg-orange-500 hover:bg-orange-600 px-6 font-black text-xs uppercase tracking-widest shadow-lg shadow-orange-100"
                    >
                        <Link href={`/recouvrement/${task.promesse_id}`}>
                           Encaisser <ArrowRight className="ml-2 h-4 w-4" />
                        </Link>
                    </Button>
                </div>
              </div>
